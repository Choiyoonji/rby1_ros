...
class MainNode(Node):
    ...
    def action_callback(self, msg):
        ...
        right_action = data[:1+7+1]  # 첫 9개: 오른팔 버튼 + 조인트 + 그리퍼
        left_action = data[1+7+1:]  # 뒷 9개: 왼팔 버튼 + 조인트 + 그리퍼
        self.main_state.is_controller_button_right = bool(right_action[0] > 0.5)
        self.main_state.is_controller_button_left = bool(left_action[0] > 0.5)
        self.main_state.desired_joint_positions = np.array(right_action[1:8]+left_action[1:8])
        self.main_state.desired_right_gripper_position = right_action[8]
        self.main_state.desired_left_gripper_position = left_action[8]
        
        self.main_state.is_controller_connected = (self.main_state.is_controller_button_right or self.main_state.is_controller_button_left)
    ...
    def publish_command(self):
        if self.main_state.is_robot_connected:  
            ...
            command_msg.left_btn = self.main_state.is_controller_button_left
            command_msg.right_btn = self.main_state.is_controller_button_right
